...
interface AprovacaoModalProps {
  isOpen: boolean;
  onClose: () => void;
  onConfirm: (planoId: string) => Promise<void>;
...
export default function AprovacaoModal({
  ...
  const handleConfirm = async () => {
    ...
    setLoading(true);
    try {
      await onConfirm(selectedPlanoId);
      onClose();
    } catch (error) {
              ...
              <Dialog.Panel className="relative transform overflow-hidden rounded-lg bg-white px-4 pb-4 pt-5 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg sm:p-6">
                  ...
                  <div className="mt-3 text-center sm:mt-0 sm:text-left w-full">
                    ...
                    <div className="mt-4 space-y-4">
                      <div>
                        ...
                        <p className="mt-1 text-sm text-gray-900">{solicitacao.empresa_nome}</p>
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-gray-700">
                          CNPJ
                        </label>
                        <p className="mt-1 text-sm text-gray-900">{solicitacao.cnpj}</p>
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-gray-700">
                          E-mail
                        </label>
                        <p className="mt-1 text-sm text-gray-900">{solicitacao.email}</p>
                      </div>
                      <div>
                ...
                <div className="mt-5 sm:mt-4 sm:flex sm:flex-row-reverse">
                  <button
                    ...
                    disabled={loading || !selectedPlanoId}
                  >
